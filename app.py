...
DB_PATH = VIEWER_DIR / 'application.json'
...
class EmbeddedHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/library.html':
            ...
            return
        if self.path == '/application.json' or self.path.startswith('/images/'):
            if self.path == '/application.json':
                self.send_response(200)
