...
def download_manga(manga_url: str) -> None:
    ...
    if not homepage:
        ...
        return
    title = homepage.find('div', class_='post-title').text.strip()
    thumbnail_url = homepage.find('div', class_='summary_image').find('img')['data-src'].strip()
    episodes_refs: list[dict] = [{
        'ep': re.search(r'\d+', chapter.text).group(),
        'link': chapter.find('a')['href']
    } for chapter in homepage.find('ul', class_='main').find_all('li', class_='wp-manga-chapter')]
    ...
    for episode_ref in reversed(episodes_refs):
        ...
        tags = chapter.find('div', class_='reading-content').find('div', class_='text-center').find_all()
        img_idx = 0
        raw_entries: list[dict] = []
        for tag in tags:
            if tag.name == 'img':
                raw_entries.append({'type': 'img', 'url': tag['src'].strip()})
        ...
        for entry in raw_entries:
            ...
            idx_str = str(img_idx).zfill(pad)
            if entry['type'] == 'img':
                out_path = img_dir / f'{idx_str}.jpg'
                try:
                    download_plain_image(entry['url'], episode_ref['link'], out_path)
                    local_images.append(f'images/{key}/{ep_num}/{idx_str}.jpg')
                    img_idx += 1
                except Exception as e:
                    print(f"\n    [WARN] Image {img_idx} failed: {e}")
